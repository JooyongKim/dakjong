"""
Direct strategy testing without data loader (no yfinance dependency).
"""
...
print("="*80)
print("투자 전략 신호 발생기 통합 테스트 (Mock 데이터)")
print("="*80)
...
print("-" * 80)
print("테스트 1: 시장 패닉 식별 및 저점 매수 (Strategy 1)")
print("-" * 80)
try:
    ...
    print(f"  전략명: {latest['strategy_name']}")
...
print("-" * 80)
print("테스트 2: 변동성/하락 속도 기반 위험 관리 (Strategy 2)")
print("-" * 80)
try:
    spy_data = create_mock_market_data(500)
...
print("-" * 80)
print("테스트 3: 추세 기반 레버리지 (Strategy 3)")
print("-" * 80)
try:
    spy_data = create_mock_market_data(500)
...
print("-" * 80)
print("테스트 4: 섹터 로테이션 (Strategy 4)")
print("-" * 80)
try:
    spy_data = create_mock_market_data(500)
    ...
    if latest['signal'] == 'HOLD_SECTORS' and 'sectors_list' in latest['details']:
        sectors = latest['details']['sectors_list']
...
print("-" * 80)
print("테스트 5: MACD-V 변동성 정규화 모멘텀 (Strategy 5)")
print("-" * 80)
try:
...
print("✓✓✓ 전체 테스트 완료! ✓✓✓")
print("="*80)
print("\n모든 5개 전략이 정상적으로 동작합니다!")
print("\n각 전략별 특징:")
print("  • Strategy 1: 시장 패닉 시 BUY 신호")
print("  • Strategy 2: 위험 증가 시 RISK_OFF 신호")
print("  • Strategy 3: 추세에 따라 LEVERAGE_ON/OFF 신호")
print("  • Strategy 4: 월/분기별 섹터 선정 및 HOLD/CASH 신호")
print("  • Strategy 5: MACD-V 기반 추세 강도 신호")
print("\n실제 사용 방법:")
